...
FORMAT_DATE        = 8
FORMAT_JOURNAL     = 9
FORMAT_UNIVERSITY  = 10
FORMAT_ABSTRACTS   = 11
FORMAT_CITES       = 12
FORMAT_NUMCITES    = 13

#Busca por año y con 20 resultados por request
def descarga(searchstr, outformat=FORMAT_BIBTEX, allresults=False, startNum=0, year=2015):
    logging.debug("Query: {sstring}".format(sstring=searchstr))
    searchstr = '/scholar?start='+str(startNum)+'&q='+quote(searchstr)+'&hl=es&start='+str(startNum)+'&as_sdt=0&as_ylo='+str(year)+'&as_yhi='+str(year)+'&as_vis=1'
...
def query(searchstr, outformat=FORMAT_BIBTEX, allresults=False):
    ...
    allresults : bool, optionalFORMAT_TITLE
...
#-----------------------------------------------------------------------
    elif outformat == FORMAT_TITLE:
        refre = re.compile(r'<h3 class="gs_rt">(.*?)\</h3\>')
    elif outformat == FORMAT_DATE:
        refre = re.compile(r'<div class="gs_a">(.*?)\</div\>')
    elif outformat == FORMAT_JOURNAL:
        refre = re.compile(r'<div class="gs_a">(.*?)\</div\>')
    elif outformat == FORMAT_UNIVERSITY:
        refre = re.compile(r'<div class="gs_a">(.*?)\</div\>')
    elif outformat == FORMAT_AUTHORS:
        refre = re.compile(r'<div class="gs_a">(.*?)\</div\>')
    elif outformat == FORMAT_ABSTRACTS:
        refre = re.compile(r'<div class="gs_rs">(.*?)\</div\>')
    elif outformat == FORMAT_CITES:
        refre = re.compile(r'<a href="(/scholar\?cites[^"]*)"')
    elif outformat == FORMAT_NUMCITES:
        refre = re.compile(r'<a href="/scholar\?cites.*?(\d+)</a>')
#-----------------------------------------------------------------------
...
def articles_Elements(text):
	# cortar archivo
	todohtml = open(text, "r").read().split('<div class="gs_r">')[1:]
	datosFiltrados = list()
	for html in todohtml:
		#print (html)
		#html        = html.decode('utf8')
		titles       = get_links( html, FORMAT_TITLE     )
		authors      = get_links( html, FORMAT_AUTHORS   )
		dates        = get_links( html, FORMAT_DATE      )
		journals     = get_links( html, FORMAT_JOURNAL   )
		university   = get_links( html, FORMAT_UNIVERSITY)
		bibText      = get_links( html, FORMAT_BIBTEX    )
		abstracts    = get_links( html, FORMAT_ABSTRACTS )
		citations    = get_links( html, FORMAT_CITES     )
		numCitations = get_links( html, FORMAT_NUMCITES  )
		...
		#print ("Number of citation:", len(numCitations))
		autoresConPerfil = 0
		i = 0  # Ahora solo hay un elemento en la lista!!!!

		title = titles[i]
		# pezca del url
		urlstart = title.find("href=\"")+6
		urlend = title.find("\"", urlstart)
		url = title[urlstart:urlend]

		#pezca de titulo
		tstart = title.find(">", urlend)+1
		tend   = title.find("</a>", tstart)
		title  = title[tstart:tend]

		#pezca año
		refre = re.compile(r'[,-] [0-9]{4} -')
		try:    ano = refre.findall(authors[i])[-1][2:7]
		except: ano = '0000'

		#pezca revista
		refre = re.compile(r'- …?(.*),')
		try:    
			revista = refre.findall(authors[i])[-1]
			if revista[:9] == "US Patent": revista = "US Patent"
			#print ("revista: ", revista)
		except:
			#print ("truco: ", authors[i].split(' - ')[-1])
			#refre = re.compile(r'- …?(.*),')
			#revista = '(sin revista)'
			revista = authors[i].split(' - ')[-1]
		#if revista == '(sin revista)':print (revista)

		#pezca de autores
		astart = 0
		if revista == '(sin revista)': 
			if ano == '0000':  aend   = authors[i].find(' - ', astart)
			else:              aend   = authors[i].find(ano, astart)-3
		else:                          aend   = authors[i].find(revista, astart)-3
		auth   = authors[i][astart:aend].split(', ')

		#pezca de editorial
		edstart   = authors[i].find(ano)+len(ano)+2
		editorial = authors[i][edstart:]

		# cantidad de citas y url de citas
		if numCitations == []:
			numCitations = 0	
			citations = 'None'
		else:
			numCitations = numCitations[0]
			#print (numCitations)
			citations = citations[0]

		titles[i] = title
		#print (urlstart, urlend)
		datosFiltrados.append([url,title,revista,ano,editorial,numCitations,citations,auth]) 
		#print (citations)
		if authors[i].find("citations") > -1: autoresConPerfil += 1

		#print ("autores con perfil: ", autoresConPerfil)
